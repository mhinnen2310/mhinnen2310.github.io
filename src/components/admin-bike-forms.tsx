...
type AdminBike = {
  ...
  currentBattery: { id: string; assetCode: string; status: string; manufacturer: string | null; model: string | null; serialNumber: string | null; nominalWh: number | null; labelPhotoKey: string | null } | null;
};
...
function IntakeProgress({ bike, intakeReadiness, onChange }: { bike: AdminBike; intakeReadiness: { ready: boolean; missing: string[] }; onChange: (tab: IntakeTab) => void }) {
...
function FeatureChips({ selected }: { selected: string[] }) {
  ...
  return <div className="mt-2 flex flex-wrap gap-2">{FEATURES.map(([key, label]) => <span key={key}><button type="button" aria-pressed={values.includes(key)} onClick={() => toggle(key)} className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${values.includes(key) ? "border-brand-700 bg-brand-50 text-brand-800" : "border-line bg-card text-ink-soft hover:bg-surface"}`}>{values.includes(key) && <span aria-hidden className="mr-1">✓</span>}{label}</button>{values.includes(key) && <input type="hidden" name="features" value={key} />}</span>)}</div>;
}
...
function collectDossier(form: FormData): { data?: Record<string, unknown>; error?: string } {
  ...
  for (const name of textFields) { if (form.has(name)) data[name] = form.get(name); }
  ...
  for (const name of integerFields) { if (!form.has(name)) continue; const value = optionalInteger(form, name); if (value === "invalid") return { error: "Controleer de hele getallen in het dossier." }; data[name] = value; }
  for (const name of ["wheelSizeInches", "batteryAh", "batteryMeasuredAh", "batterySohPercent"]) { if (!form.has(name)) continue; const value = optionalDecimal(form, name); if (value === "invalid") return { error: "Controleer wielmaat en accucapaciteit." }; data[name] = value; }
  const priceCents = euroToCents(form.get("priceEuro"));
  ...
  if (form.has("isElectric")) data.isElectric = optionalBoolean(form, "isElectric");
  if (form.has("walkAssist")) data.walkAssist = optionalBoolean(form, "walkAssist");
  if (form.has("batteryReconditioned")) data.batteryReconditioned = optionalBoolean(form, "batteryReconditioned");
  if (form.has("acquisitionDate")) data.acquisitionDate = form.get("acquisitionDate") || null;
  if (form.has("batteryRevisionDate")) data.batteryRevisionDate = form.get("batteryRevisionDate") || null;
  if (form.has("batteryTestDate")) data.batteryTestDate = form.get("batteryTestDate") || null;
...
export function AdminBikeEditor({ bike, margin, auditEvents, intakeReadiness, workshopReadiness }: { bike: AdminBike; margin: Margin; auditEvents: AuditEvent[]; intakeReadiness: { ready: boolean; missing: string[] }; workshopReadiness: { ready: boolean; missing: string[] } }) {
  ...
  const [activeTab, setActiveTab] = useState<IntakeTab>("overview");
  ...
  return <div className="max-w-6xl"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-ink-faint">{bike.inventoryCode} · intake {formatDate(bike.acquisitionDate ?? bike.createdAt)}</p><h2 className="text-2xl font-bold tracking-tight text-ink">{bike.title}</h2></div>{lifecycleManaged ? <p className="rounded-lg bg-surface px-3 py-2 text-sm text-ink-soft">Status: <strong>{bike.status === "RESERVED" ? "Gereserveerd" : "Verkoop wordt afgerond"}</strong><br /><span className="text-xs">Deze status wordt door de reserverings- en verkooplifecycle beheerd.</span></p> : <form onSubmit={changeStatus} className="flex items-end gap-2"><label className="text-sm text-ink-soft">Status<select name="status" defaultValue={bike.status === "SOLD" ? "ARCHIVED" : bike.status} className="mt-1 block rounded-lg border border-line bg-card px-3 py-2 text-sm text-ink">{statusChoices.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button type="submit" disabled={busy !== null} className="rounded-lg border border-brand-700 px-3 py-2 text-sm font-semibold text-brand-800 hover:bg-brand-50 disabled:opacity-70">{busy === "status" ? "Wijzigen…" : bike.status === "SOLD" ? "Archiveren" : "Status wijzigen"}</button></form>}</div>
    ...
    <div hidden={activeTab !== "overview"} className="mt-6"><BikeIntakePanel bikeId={bike.id} intake={bike.intakeRecord} batteryLinked={Boolean(bike.currentBattery)} readiness={intakeReadiness} /></div>
    <div hidden={activeTab !== "condition"} className="mt-6"><BikeWorkshopPanel bikeId={bike.id} tasks={bike.serviceTasks} readiness={workshopReadiness} /></div>
    <div hidden={activeTab !== "battery"} className="mt-6 grid gap-6 lg:grid-cols-2"><BatteryDossierPanel bikeId={bike.id} currentBattery={bike.currentBattery} battery={{ manufacturer: bike.batteryManufacturer, model: bike.batteryModel, voltage: bike.batteryVoltage, nominalAh: bike.batteryAh, nominalWh: bike.batteryWh, measuredAh: bike.batteryMeasuredAh, measuredWh: bike.batteryMeasuredWh, sohPercent: bike.batterySohPercent, testDate: bike.batteryTestDate, testMethod: bike.batteryTestMethod, cycles: bike.batteryCycleCount, serial: bike.batterySerialRef, reconditioned: bike.batteryReconditioned, warrantyMonths: bike.batteryWarrantyMonths }} /><BatteryLabelPanel bikeId={bike.id} batteryId={bike.currentBattery?.id} photoKey={bike.currentBattery?.labelPhotoKey ?? bike.batteryLabelPhotoKey} /></div>
    <div hidden={activeTab !== "overview"} className="mt-6 grid gap-6 lg:grid-cols-2">
