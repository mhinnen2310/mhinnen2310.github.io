...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  } catch (error) { if (error instanceof QrTagError) return NextResponse.json({ error: error.message }, { status: 400 }); return NextResponse.json({ error: "Correctie kon niet worden verwerkt." }, { status: 500 }); }
}
