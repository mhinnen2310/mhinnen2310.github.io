...
export default async function AdminBatteriesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  // Older imports stored battery fields on Bike. Promote those records the
  // first time the separate battery register is opened, without deleting the
  // legacy snapshot.
  const legacyBikes = await prisma.bike.findMany({ where: { currentBatteryId: null, OR: [{ batteryType: { not: null } }, { batteryManufacturer: { not: null } }, { batteryModel: { not: null } }, { batteryWh: { not: null } }, { batterySerialRef: { not: null } }] }, select: { id: true }, take: 500 });
  for (const bike of legacyBikes) await ensureLegacyBatteryForBike(bike.id).catch(() => null);
