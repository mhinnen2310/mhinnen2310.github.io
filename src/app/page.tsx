...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-14">
      {/* Hero */}
      <section className="border-b border-line bg-card" aria-labelledby="hero-heading">
        <div className="mx-auto grid max-w-6xl items-center gap-8 px-4 py-10 md:grid-cols-2 md:py-14">
          <div>
            ...
            <h1 id="hero-heading" className="mt-4 text-3xl font-bold tracking-tight text-ink sm:text-4xl">
              {heroTitle}
            </h1>
            <p className="mt-4 max-w-prose text-base leading-relaxed text-ink-soft">{heroSubtitle}</p>
          ...
          <div className="relative">
            {heroBike?.coverImage ? (
              ...
              <div className="flex aspect-[4/3] w-full items-center justify-center rounded-2xl border border-line bg-brand-50" role="img" aria-label="Demi Fietsen">
                <BikeIcon />
              </div>
            )}
          </div>
      ...
      {/* Why us */}
      <section aria-labelledby="why-heading" className="mx-auto max-w-6xl px-4">
        <h2 id="why-heading" className="text-xl font-semibold tracking-tight text-ink">
        ...
        {warranty.enabled && (
          <p className="mt-4 rounded-lg border border-line bg-card px-4 py-3 text-sm text-ink-soft">
            <strong className="font-semibold text-ink">Garantie: </strong>
            {warranty.publicNote}
          </p>
        )}
      </section>
      ...
      {/* Current inventory */}
      <section aria-labelledby="stock-heading" className="mx-auto max-w-6xl px-4">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 id="stock-heading" className="text-xl font-semibold tracking-tight text-ink">
              Nu beschikbaar
            </h2>
            <p className="mt-1 text-sm text-ink-soft">
              Elke fiets is uniek: de foto’s en specificaties horen bij deze specifieke fiets.
            </p>
          </div>
          <Link href="/fietsen" className="text-sm font-semibold text-brand-700 hover:text-brand-800 hover:underline">
            Bekijk het volledige aanbod →
          </Link>
        </div>
        {bikes.length === 0 ? (
          <div className="mt-5 rounded-xl border border-dashed border-line bg-card px-6 py-10 text-center">
            <p className="text-base font-semibold text-ink">Op dit moment geen fietsen online</p>
            <p className="mt-1 text-sm text-ink-soft">
              Er komen regelmatig nieuwe fietsen binnen. Kom langs of bel even — misschien is er iets wat nog niet online staat.
            </p>
            <div className="mt-4 flex justify-center gap-3">
              <Link href="/contact" className="rounded-lg border border-brand-300 px-4 py-2 text-sm font-medium text-brand-800 hover:bg-brand-50">
                Neem contact op
              </Link>
              <Link href="/afspraak" className="rounded-lg bg-brand-700 px-4 py-2 text-sm font-medium text-white hover:bg-brand-800">
                Plan een proefrit
              </Link>
            </div>
          </div>
      ...
      {/* How it works */}
      <section aria-labelledby="how-heading" className="border-y border-line bg-card">
        <div className="mx-auto max-w-6xl px-4 py-10">
          <h2 id="how-heading" className="text-xl font-semibold tracking-tight text-ink">
            Zo werkt het
          </h2>
          <ol className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { step: "1", title: "Vind je fiets", body: "Blader door het actuele aanbod en bekijk de specificaties van elke unieke fiets." },
              { step: "2", title: "Bekijk & proefrijd", body: "Maak een afspraak en rijd op de fiets die je ziet — precies dit exemplaar." },
              { step: "3", title: "Koop of reserveer", body: "Bestel online of koop direct bij ophaling. Betaal veilig met iDEAL of creditcard." },
              { step: "4", title: "Ophalen of leveren", body: "Haal je fiets bij ons op of laat hem bezorgen. Met factuur en garantie." },
            ].map((s) => (
              <li key={s.step} className="rounded-xl border border-line bg-surface p-4">
                <span aria-hidden className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-700 text-sm font-bold text-white">
                  {s.step}
                </span>
      ...
      {/* About teaser */}
      <section aria-labelledby="about-heading" className="mx-auto max-w-6xl px-4">
        <div className="flex flex-col gap-6 rounded-2xl border border-line bg-brand-50 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
          <div className="max-w-2xl">
            <h2 id="about-heading" className="text-xl font-semibold tracking-tight text-brand-900">
              Over Demi Fietsen
            </h2>
      ...
      {settings.newsletterEnabled && (
        <section aria-labelledby="news-heading" className="mx-auto max-w-6xl px-4">
          <div className="rounded-2xl border border-line bg-card p-6 sm:p-8">
            <h2 id="news-heading" className="text-lg font-semibold text-ink">
              Nieuwste fietsen eerst in je inbox
            </h2>
            <p className="mt-1 max-w-prose text-sm text-ink-soft">
              We mailen alleen wanneer er iets nieuws binnenkomt. Geen spam, en afmelden kan met één klik.
            </p>
            <Link
              href="/nieuwsbrief"
...
function BikeIcon() {
  return (
    <svg width="120" height="90" viewBox="0 0 120 90" fill="none" aria-hidden className="text-brand-600">
      <circle cx="28" cy="62" r="20" stroke="currentColor" strokeWidth="4" />
      <circle cx="92" cy="62" r="20" stroke="currentColor" strokeWidth="4" />
      <path d="M28 62L48 30h24l20 32M48 30l12 32h-8M72 30l-12 32M40 22h16" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
